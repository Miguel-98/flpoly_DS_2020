...
```{r}
now_doing <- paste(my_name, "is awesome")
```
...
is_twice_as_long <- function(str1, str2){
  ...
  #returns true or false if difference is more than short one
  diff >= min_length
}
...
```{r}
is_twice_as_long("dog", "elefant")
is_twice_as_long("elefant", "dog")
...
describe_difference <- function(first, second){
  ...
  if(diff>0){
    ...
    sentence <- paste("Your second word is longer by",-diff, "characters")
  } else{
    sentence <- paste("Words are the same length")
  }
  return(sentence)
}
...
describe_difference <- function(first, second){
  ...
  if(diff>0){
    ...
    paste("Your second word is longer by",-diff, "characters")
  } else{
    paste("Words are the same length")
  }
